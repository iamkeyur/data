  ...
  List.fold_left
	...
	Sys.command
	  (Printf.sprintf "cd %s; git show %s:%s > %s"
			...
			let cmd1 =
				Printf.sprintf
					"python delog.py %s" btmp in
			let cmd2 =
				Printf.sprintf
					"python delog.py %s" atmp in
			let results1 = C.cmd_to_list cmd1 in
			Printf.eprintf "results: %d\n" (List.length results1);
			let results2 = C.cmd_to_list cmd2 in
			Printf.eprintf "results: %d\n" (List.length results2);
